...
const ChatMessage = React.forwardRef<HTMLDivElement, MessageProps>(({ message, same }, ref?) => {
  ...
  return (
    /* Chat Message */
    <div ref={ref} className={cn('flex items-end ', { 'justify-end': isUser })}>
      <div className={cn('relative flex items-center justify-center h-6 w-6 aspect-square rounded-sm', isUser ? 'order-2 bg-blue-600 dark:bg-gray-400' : 'order-1 bg-zinc-800 dark:bg-slate-100', { invisible: same })}>
        {/* */}
        {isUser ? <Icons.user className="w-3/4 h-3/4 fill-zinc-200 text-zinc-200 dark:fill-slate-700 dark:text-slate-700" /> : <Icons.logo className="w-3/4 h-3/4 fill-zinc-200 dark:fill-slate-700" />}
      </div>
